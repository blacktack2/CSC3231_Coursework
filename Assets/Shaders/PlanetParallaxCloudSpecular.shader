...
Shader "Shaders/Planet Parallax Cloud Specular"
{
	Properties
	{
		...
		Pass
		{
			...
			CGPROGRAM
				...
				half4 frag( vertOut IN ) : COLOR
				{
					...
					// Modified clouds to use the same parallax as the ground
					half4 atmos			= tex2D( _CloudMap, IN.uv_GroundMap );
					float3 atmosColor = (atmos * _CloudAlpha).rgb * _LightColor0.rgb * diffGround;
